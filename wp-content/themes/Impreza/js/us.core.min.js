    ...
    setup: function (_, ns, handle) {
      ...
      if (!!self.addEventListener) {
        self.addEventListener(type, handle, {
          passive:
            type.indexOf("touch") === 0 ? !ns.includes("noPreventDefault") : !0,
        });
      }
    },
  };
      ...
      scroll: function () {
        ...
        for (var i = 0; i < self.waypoints.length; i++) {
          if (self.waypoints[i].scrollPos < scrollTop) {
            self.waypoints[i].fn(self.waypoints[i].$elm);
...
(function ($) {
  ...
  var USAnimate = function (container) {
    ...
    this.$items = $('[class*="us_animate_"]', this.$container).not(
      ".off_autostart"
    );
    ...
    _validExtension: function (file, accepts) {
      ...
      for (var i in accepts) {
        ...
        if (accept.indexOf("/") > -1) {
          ...
          self.getExtension(file.name) === accept.replace(/[^A-z\d]+/, "")
        ) {
          return !0;
        }
      }
      ...
      if (input.files.length) {
        for (var i in input.files) {
          ...
          if (!self._validExtension(file, accept)) {
            errMessage = (self.options.messages.err_extension || "").replace(
              "%s",
              self.getExtension(file.name)
            );
          }
          if (!errMessage && file.size > parseFloat(maxSize) * 1048576) {
            errMessage = (self.options.messages.err_size || "").replace(
              "%s",
              maxSize
            );
          }
                ...
                if (status == "OK") {
                  ...
                  if (that.options.markers[i].infowindow) {
                    marker.infoWindow.open(
                      $.extend({}, that.GMapsObj.map, { shouldFocus: !1 }),
                      marker
                    );
                  }
                  $us.WMapsCurrentGeocode++;
          ...
          function () {
            ...
            $.each(
              ...
              function (breakpointWidth, breakpointArgs) {
                if (
                  breakpointArgs !== _undefined &&
                  breakpointArgs.items !== _undefined
                ) {
  ...
  $.extend($us.WGridFilter.prototype, $us.mixins.Events, {
    init: function (container, options) {
      ...
      if (this.$container.hasClass("togglable")) {
        this.$filtersItem.on(
          "click",
          ".w-filter-item-title",
          this._events.showHideAccordionItem.bind(this)
        );
      }
    },
    isMobile: function () {
    ...
    _events: {
      ...
      toggleItemValue: function (_, item) {
        ...
        if (this.$container.hasClass("togglable") && hasValue) {
          $item.addClass("open");
        }
        $("> .w-filter-item-title > span:not(.w-filter-item-reset)", item).html(
          title
        );
      },
      resize: function () {
  ...
  $.extend(USHeader.prototype, $us.mixins.Events, {
    ...
    isStickyAutoHideEnabled: function () {
      ...
      return (
        ...
        (self.settings[self.state].options.sticky_auto_hide || !1)
      );
    },
    isSticky: function () {
    ...
    getHeight: function () {
      ...
      return !isNaN(height) ? $us.parseFloat(height) : 0;
    },
        ...
        if (
          ...
          if (self.isStickyEnabled()) {
            var scrollBreakpoint =
                parseInt(self.settings[self.state].options.scroll_breakpoint) ||
                100,
...
!(function ($) {
  ...
  $us.WLmaps.prototype = {
    ...
    redraw: function () {
      ...
      $us.timeout(self.lmap.invalidateSize.bind(self.lmap, !0), 100);
    },
    ...
    renderMap: function () {
      ...
      if (
        $us.usbPreview() ||
        (this.options.disableDragging && !$us.$html.hasClass("no-touch"))
      ) {
...
(function ($, undefined) {
  ...
  $us.PageScroller.prototype = {
    ...
    _init: function () {
      ...
      $("> *:not(.l-header) .l-section", $us.$canvas).each(
        function (key, elm) {
          var $section = $(elm),
          ...
          if (
            isPopupContent ||
            ($section.parents(".l-section").length && !isPopupWrapper)
          ) {
            return;
          }
          var section = {
      ...
      if (
        ...
        if (
          ...
          if (
            ...
            var rowIndex =
              this.sections[this.lastContentSectionIndex + key] !== _undefined
...
!(function ($) {
  ...
  $us.UsSharing.prototype = {
    ...
    showTooltip: function (e) {
      $(".w-sharing-item", this.$container).each(
        function (index, elm) {
          ...
          $(elm).attr(
            "href",
            ($(elm).data("url") || "").replace("{{text}}", this.selectedText)
          );
        }.bind(this)
      );
      this.$container.css({
    ...
    setSharingImage: function () {
      $(".w-sharing-item", this.$container).each(
        function (index, elm) {
          ...
          $(elm).attr(
            "href",
            ($(elm).attr("href") || "").replace("{{image}}", this.sharingImage)
          );
    ...
    copyToClipboard: function () {
      ...
      if (
        this.$copy2clipboard.parent().data("sharing-url") !== undefined &&
  ...
  $us.WTabs.prototype = {
    init: function (container, options) {
      ...
      if ($us.usbPreview()) {
        ...
        this.$container.on(
          "usb.contentChange",
          $us.debounce(usbContentChange, 1)
        );
      }
    },
    isTrendy: function () {
...
(function ($, undefined) {
  ...
  $us.wVideo = function (container) {
    ...
    this._events = { overlayClick: this._overlayClick.bind(this) };
  ...
  $.extend($us.wVideo.prototype, {
    ...
    insertPlayer: function () {
      ...
      if (
        data.player_api &&
        !$('script[src="' + data.player_api + '"]', _document.head).length
      ) {
...
!(function ($, _undefined) {
  ...
  var USWooCommerce = function () {
    ...
    if (this.isCartPage()) {
      ...
      $.ajaxPrefilter(
        function (_, originalOptions, jqXHR) {
          ...
          if (data.indexOf("&us_calc_shipping") > -1) {
            jqXHR.done(function (res) {
              $(".w-cart-shipping .woocommerce-shipping-destination").html(
                $(
  ...
  $.extend(USWooCommerce.prototype, {
    ...
    _switchCart: function () {
      this.$cart.on("focus.upsolution blur.upsolution", function (e) {
        $(e.target)[e.type === "focus.upsolution" ? "addClass" : "removeClass"](
          "opened"
        );
      });
    },
